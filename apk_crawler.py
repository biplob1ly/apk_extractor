...
firm_sys_hash_file = os.path.expanduser("huawei_sys_hash.txt")
log = "log starts here.\n"
start = None
apk_dict = {}
apk_cert_dict = {}
...
def get_modulus_and_exponent(x509):
    if x509.get_pubkey().type() == TYPE_RSA:
        ...
        pub_key = ''.join(['%02x' % ch for ch in pub_der[1]])
        ...
        # print(public_key + "\n" + modulus + "\n" + str(exponent))
        return [modulus, exponent, pub_key]
    return ''
    ...
    try:
        ...
        if apk_dict[source] == 'firmware':
            ...
            if cursor.rowcount == 0:
                ...
                # if get_build_prop(apk_dict[extract_dir]):
                ...
                get_build_prop(apk_dict[extract_dir])
                ...
                apk_dict[source_url] = "official_website"
...
def get_build_prop(firm_dir):
    item_count = len(build_prop_table)
    file_name = os.path.join(firm_dir, "build.prop")
...
def get_apk_info(apk_path, cert_destination, apk_source):
    ...
    if apk_source == "firmware":
        apk_path_items = apk_path.split("/system/")
        apk_dict[extract_dir] = apk_path_items[0]
        apk_dict[firm_name] = os.path.basename(apk_path_items[0])
        apk_dict[apk_filepath] = "/system/" + apk_path_items[1]
        with open(firm_sys_hash_file, 'r') as fin:
            for line in fin:
                if apk_dict[firm_name] in line:
                    items = line.rstrip().split(',')
                    apk_dict[firm_md5] = items[0]
                    apk_dict[firm_sha1] = items[1]
                    apk_dict[firm_sha256] = items[2]
                    apk_dict[system_md5] = items[3]
    ...
    try:
        ...
        if db_connection.is_connected():
            ...
            for apk_hash_path in apk_hash_paths:
                initialize_dicts()
                contents = apk_hash_path.split(',')
                for index, item in enumerate(apk_hash_path_items):
                    apk_dict[item] = contents[index]
                print(str(start) + " : " + str(count) + " -> " + apk_dict[apk_original_path])
                if not os.path.exists(apk_dict[apk_original_path]):
                    print("Apk path doesn't exist")
                else:
                    insert_apk_info_into_db(db_connection, apk_dict[apk_original_path], cert_destination, "firmware")
                count = count + 1
                start = start + 1
...
if __name__ == "__main__":
    ...
    if app_source == "play-store":
        ...
        apk_names_file = os.path.expanduser('huawei_apk_hash_path.txt')
        # dest_cert_dir = os.path.expanduser('~/Documents/bal/certs/')
        # apk_names_file = os.path.expanduser('~/Projects/apk_extractor/all_firmware_apks.txt')
        dest_cert_dir = os.path.expanduser('~/Documents/firmwareNAS/bip/certextract/certs/')
