...
firm_apk_hash_path = "motorola_apk_hash_path.txt"
firm_hash_file = "batch3_hash.txt"
source_path_file = "motorola_firm_paths.txt"
logger_file = "motorola_extraction_log.txt"
firm_pos = "motorola_firm_pos.txt"
start = None
    ...
    if mount_status == success and sys_dir is not None:
        ...
        try:
            ...
            if build_prop_path is not None:
                ...
                subprocess.call("echo '" + password + "' | sudo cat " + build_prop_path + " > "
                                + os.path.join(build_prop_dir, firmware_name.replace(' ', '\\ ')), shell=True)
            ...
            #                     + os.path.join(firmware_name.replace(' ', '\\ ')), shell=True)
        except Exception as ex:
            print(str(ex))
        ...
        try:
            with open(firm_hash_file, "a+") as hash_in:
                hash_in.write(firm_hash.values()['md5'] + ' ' + firmware_path + "\n")
