...
class DZFileTools:
    ...
    def extractPartition(self,index):
        ...
        currentPartition['name'] = currentPartition['name'].replace(b'\x00', b'')
    ...
    def openFile(self, dzfile):
        ...
        if verify_header != self.dz_header:
            ...
            print("[ ] Expected: %s ,\n\tbut received %s ." % ("".join("%02x" % n for n in self.dz_header), "".join("%02x" % n for n in verify_header)))
            sys.exit(0)
    ...
    def cmdExtractAll(self):
        ...
        for part in enumerate(self.partList):
            # print("[+] Extracting " + str(part[1][0], "utf-8") + " to " + os.path.join(self.outdir,str(part[1][0], "utf-8")))
